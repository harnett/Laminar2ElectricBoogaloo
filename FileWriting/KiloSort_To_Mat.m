function unitdata = KiloSort_To_Mat(fdir,rec_areas,chs)
...
params.excludeNoiseAndMUA = 1;

spikeStruct = loadKSdir(fdir, params);
...
for u = 1 : length(spikeStruct.cids)
    ...
    unitdata(u).area = area{unitdata(u).ch};
...
[~,pi] = sort(vertcat(unitdata(:).ch));
unitdata = unitdata(pi);
