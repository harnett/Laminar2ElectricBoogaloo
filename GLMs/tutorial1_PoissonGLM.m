...
load('samp_lfp_dat');    % stimulus (temporal binary white noise)
load('glm_SpTimes'); % load spike times (in units of stim frames)
Stim = randn(600001,1).*30;
%Stim = lfp(64,:)';
stimtimes = [0:.001:(600000*.001)]';% stim frame times in seconds (if desired)

% Pick a cell to work with
cellnum = 8; % (1-2 are OFF cells; 3-4 are ON cells).
tsp = SpTimes{cellnum}./1000;
% -------------------------------------------------------------------------
...
subplot(211);
iiplot = 1:1200; % bins of stimulus to plot
